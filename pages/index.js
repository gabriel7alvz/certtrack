      ...
      <Draggable draggableId={ind._firebaseId} index={index}>
        {(provided, snapshot) => (
          ...
          >
            ...
            {(ind.cpf || ind.cnpj) && <div style={{ color: "#aaa", fontSize: 10 }}>{ind.cpf ? `CPF: ${ind.cpf}` : `CNPJ: ${ind.cnpj}`}</div>}
        ...
        <div style={{ position: "fixed", inset: 0, background: "#00000060", zIndex: 1000, display: "flex", alignItems: "center", justifyContent: "center", padding: 24 }}>
          <div style={{ background: "#fff", border: "1px solid #E0E7E0", borderRadius: 20, padding: "28px", width: "100%", maxWidth: 420, boxShadow: "0 8px 32px #1B2E4B20" }}>
            ...
            <div style={{ display: "flex", gap: 10, marginTop: 16 }}>
              ...
              <button onClick={confirmarCancelamento} style={{ flex: 1, padding: "12px", borderRadius: 10, border: "none", background: "#C62828", color: "#fff", fontSize: 14, fontWeight: 700, cursor: "pointer" }}>Confirmar</button>
            </div>
          </div>
        </div>
      )}
        ...
        <div style={{ background: "#fff", borderBottom: "1px solid #E0E7E0", padding: "0 32px" }}>
          <div style={{ maxWidth: "100%", margin: "0 auto", height: 64, display: "flex", alignItems: "center", justifyContent: "space-between" }}>
            ...
            <div style={{ display: "flex", gap: 8 }}>
              <button onClick={refresh} style={{ background: "#F5F7FA", border: "1px solid #E0E7E0", color: "#666", borderRadius: 8, padding: "7px 14px", fontSize: 13, cursor: "pointer" }}>↻</button>
              <select value={mes} onChange={e => setMes(e.target.value)} style={{ background: "#F5F7FA", border: "1px solid #E0E7E0", color: "#1B2E4B", borderRadius: 8, padding: "7px 12px", fontSize: 13, cursor: "pointer" }}>
              ...
              <button onClick={logout} style={{ background: "#FFF0F0", border: "1px solid #FFCDD2", color: "#C62828", borderRadius: 8, padding: "7px 14px", fontSize: 13, cursor: "pointer" }}>Sair</button>
            </div>
          </div>
        </div>
              ...
              <div style={{ display: "flex", gap: 12, overflowX: "auto", paddingBottom: 16, alignItems: "flex-start" }}>
                  ...
                  return (
                    <div key={col.id} style={{ minWidth: 240, maxWidth: 240, flexShrink: 0 }}>
                      <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 12 }}>
                        ...
                        <span style={{ background: col.bg, color: col.color, border: "1px solid " + col.border, borderRadius: 20, padding: "2px 8px", fontSize: 11, fontWeight: 700, marginLeft: "auto" }}>{cards.length}</span>
                      </div>
                ...
                <div style={{ minWidth: 240, maxWidth: 240, flexShrink: 0 }}>
                  <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 12 }}>
                    ...
                    <span style={{ background: "#FFF5F5", color: "#C62828", border: "1px solid #FFCDD2", borderRadius: 20, padding: "2px 8px", fontSize: 11, fontWeight: 700, marginLeft: "auto" }}>{canceladas.length}</span>
                  </div>
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: 20, maxWidth: 600 }}>
              <div style={{ ...cardStyle, padding: 28 }}>
                <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 22 }}>
                  ...
                  {!editingNames
                    ? <button onClick={() => setEditNomes(true)} style={{ background: "#F5F7FA", border: "1px solid #E0E7E0", color: "#666", borderRadius: 8, padding: "8px 16px", fontSize: 13, cursor: "pointer" }}>Editar</button>
                ...
                {PARTNERS.map(p => (
                  <div key={p.id} style={{ display: "flex", alignItems: "center", gap: 14, marginBottom: 14 }}>
                    ...
                    {editingNames
                      ? <input value={nomes[p.id] || ""} onChange={e => setNomes(prev => ({ ...prev, [p.id]: e.target.value }))} style={{ background: "#F5F7FA", border: "1px solid #E0E7E0", borderRadius: 8, padding: "10px 14px", color: "#1B2E4B", fontSize: 14, flex: 1, outline: "none" }} />
              ...
              <div style={{ ...cardStyle, padding: 28 }}>
                ...
                {PARTNERS.map(p => (
                  <div key={p.id} style={{ background: "#F5F7FA", border: "1px solid #E0E7E0", borderRadius: 12, padding: "16px 20px", marginBottom: 12, display: "flex", alignItems: "center", justifyContent: "space-between", gap: 16 }}>
                    <div>
