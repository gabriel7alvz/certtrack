...
export default function FormPage() {
  ...
  const submit = () => {
    ...
    saveData(data);
    setNome(""); setTelefone(""); setObs(""); setTipo("PF");
    setSuccess(true);
    setLoading(false);
  ...
  return (
    <>
      ...
      <div style={{ minHeight: "100vh", background: "#080810", display: "flex", alignItems: "center", justifyContent: "center", padding: "24px 16px" }}>
        ...
        <div className="fadeUp" style={{ width: "100%", maxWidth: 460, position: "relative", zIndex: 1 }}>
          ...
          <div style={{ background: "#10101A", border: "1px solid #1E1E2E", borderRadius: 20, padding: "30px 28px" }}>
            <div style={{ marginBottom: 20 }}>
              <label style={{ display: "block", color: "#888", fontSize: 11, fontWeight: 700, letterSpacing: 1, marginBottom: 8 }}>NOME COMPLETO</label>
              <input type="text" value={nome} placeholder="Ex: Maria da Silva" onChange={(e) => setNome(e.target.value)} style={inp}
                onFocus={(e) => (e.target.style.borderColor = c)} onBlur={(e) => (e.target.style.borderColor = "#25253A")} />
            </div>

            <div style={{ marginBottom: 20 }}>
              <label style={{ display: "block", color: "#888", fontSize: 11, fontWeight: 700, letterSpacing: 1, marginBottom: 8 }}>WHATSAPP / TELEFONE</label>
              <input type="tel" value={telefone} placeholder="(11) 99999-9999" onChange={(e) => setTelefone(e.target.value)} style={inp}
                onFocus={(e) => (e.target.style.borderColor = c)} onBlur={(e) => (e.target.style.borderColor = "#25253A")} />
            </div>

            <div style={{ marginBottom: 20 }}>
              <label style={{ display: "block", color: "#888", fontSize: 11, fontWeight: 700, letterSpacing: 1, marginBottom: 8 }}>TIPO DE CERTIFICADO</label>
              <div style={{ display: "flex", gap: 8 }}>
                {["PF", "PJ", "MEI", "NF-e"].map((t) => (
                  <button key={t} onClick={() => setTipo(t)} style={{ flex: 1, padding: "11px 0", borderRadius: 9, border: "1px solid", borderColor: tipo === t ? c : "#25253A", background: tipo === t ? c + "18" : "transparent", color: tipo === t ? c : "#555", fontWeight: 700, fontSize: 13, cursor: "pointer", transition: "all 0.2s" }}>{t}</button>
                ))}
              </div>
            </div>

            <div style={{ marginBottom: 28 }}>
              <label style={{ display: "block", color: "#888", fontSize: 11, fontWeight: 700, letterSpacing: 1, marginBottom: 8 }}>OBSERVACOES <span style={{ color: "#444", fontWeight: 400 }}>(opcional)</span></label>
              <textarea rows={3} value={obs} placeholder="Alguma info adicional..." onChange={(e) => setObs(e.target.value)}
                style={{ ...inp, resize: "vertical" }}
                onFocus={(e) => (e.target.style.borderColor = c)} onBlur={(e) => (e.target.style.borderColor = "#25253A")} />
            </div>

            <button onClick={submit} disabled={loading || !nome.trim() || !telefone.trim()} style={{ width: "100%", padding: "15px", borderRadius: 12, border: "none", background: c, color: "#fff", fontSize: 15, fontWeight: 700, cursor: "pointer", letterSpacing: 0.5, transition: "opacity 0.2s", opacity: !nome.trim() || !telefone.trim() ? 0.4 : 1 }}>
